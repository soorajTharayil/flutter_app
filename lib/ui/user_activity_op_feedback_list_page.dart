...
/// User Activity OP Feedback List Page
/// Uses API: GET view/user_activity_opf/all/{userId}
/// Displays ALL OP Feedback records for the user (no date filtering)
class UserActivityOpFeedbackListPage extends StatefulWidget {
  final String userId;
  final String tdate; // older date (start)
  final String fdate; // newer date (end)

  const UserActivityOpFeedbackListPage({
    Key? key,
  ...
  @override
  State<UserActivityOpFeedbackListPage> createState() => _UserActivityOpFeedbackListPageState();
}

class _UserActivityOpFeedbackListPageState extends State<UserActivityOpFeedbackListPage> {
  ...
  /// Fetch OP feedbacks from User Activity API
  /// API: GET view/user_activity_opf/all/{userId}
  Future<void> _fetchFeedbacks({bool forceRefresh = false}) async {
    ...
    try {
      ...
      // Call User Activity API: view/user_activity_opf/all/{userId}?tdate=YYYY-MM-DD&fdate=YYYY-MM-DD
      final apiUrl = 'https://$domain.efeedor.com/view/user_activity_opf/all/${widget.userId}?tdate=${widget.tdate}&fdate=${widget.fdate}';
      final uri = Uri.parse(apiUrl);

      print('🔵 [USER ACTIVITY OP LIST] ========================================');
      print('🔵 [USER ACTIVITY OP LIST] Calling User Activity API');
      print('🔵 [USER ACTIVITY OP LIST] API: view/user_activity_opf/all/{userId}');
      print('🔵 [USER ACTIVITY OP LIST] URL: $apiUrl');
      print('🔵 [USER ACTIVITY OP LIST] User ID: ${widget.userId}');
      print('🔵 [USER ACTIVITY OP LIST] ========================================');
      ...
      print('🟢 [USER ACTIVITY OP LIST] Response Status: ${response.statusCode}');
      print('🟢 [USER ACTIVITY OP LIST] Response Body: ${response.body}');
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          if (decoded is List) {
            ...
            } else if (decoded.containsKey('op_feedbacks') && decoded['op_feedbacks'] is List) {
              feedbacksList = decoded['op_feedbacks'] as List<dynamic>;
              ...
              for (var key in decoded.keys) {
                ...
                if (value is List && value.isNotEmpty) {
                  ...
                  if (firstItem is Map) {
                    ...
                    if (itemKeys.any((k) => ['patient_id', 'uhid', 'patient_name'].contains(k.toString().toLowerCase()))) {
                      feedbacksList = value;
                      break;
                    }
                  }
          ...
          if (finalList.isNotEmpty) {
            ...
            print('🟢 [USER ACTIVITY OP LIST] Sample first item keys: ${firstItem.keys.toList()}');
            print('🟢 [USER ACTIVITY OP LIST] Sample first item: ${firstItem}');
            
            // Log specific fields to verify structure
            print('🟢 [USER ACTIVITY OP LIST] ========================================');
            print('🟢 [USER ACTIVITY OP LIST] FIELD MAPPING VERIFICATION:');
            print('🟢 [USER ACTIVITY OP LIST]   patientid: ${firstItem['patientid']}');
            print('🟢 [USER ACTIVITY OP LIST]   dataset: ${firstItem['dataset']}');
            if (firstItem.containsKey('dataset') && firstItem['dataset'] is Map) {
              final dataset = firstItem['dataset'] as Map;
              print('🟢 [USER ACTIVITY OP LIST]   dataset.name: ${dataset['name']}');
            }
            print('🟢 [USER ACTIVITY OP LIST]   datetime: ${firstItem['datetime']}');
            print('🟢 [USER ACTIVITY OP LIST] ========================================');
          }
